...
function isPalindrome(inputString = "") {
  ...
  if (inputString.length < 1000) {
    ...
    result = checkByRecursion(inputString);
    console.timeEnd("checkByRecursion");
    logResult(inputString, result);
  }
}

function checkByReversingSecondHalf(string = "") {
...
function checkByLoop(str) {
  ...
  for (var i = 0; i < len; i++) if (str[i] !== str[str.length - i - 1]) result = false;
